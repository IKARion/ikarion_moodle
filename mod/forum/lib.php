...
function forum_add_instance($forum, $mform = null) {
    ...
    if ($forum->type == 'single') {  // Create related discussion.
        ...
        // New! Enable group mode for single threaded forums. Create one discussion per group.
        $groups = groups_get_all_groups($forum->course);
        ...
        foreach ($groups as $group) {
            ...
            $discussion->message = '<b>' . $group->name . " (" . implode(", ", array_map(function ($user) {
                    return $user->firstname . " " . $user->lastname;
                }, groups_get_members($group->id))) . '):</b><br>' . $forum->intro; // MODIFIED. Add list of group members in the first post. TH
        ...
        // Add top level discussion for the sake of compliance with the unmodified version of this module.
        $discussion = new stdClass();
        $discussion->course = $forum->course;
        $discussion->forum = $forum->id;
        $discussion->name = $forum->name;
...
function forum_update_instance($forum, $mform) {
    ...
    if ($forum->type == 'single') {  // Update related discussion and post.
        ...
        if (!empty($discussions)) {
            ...
            // !New. Enable group mode for single threaded discussion forums.
            foreach ($groups as $group) {
...
function forum_get_discussions($cm, $forumsort = "", $fullpost = true, $unused = -1, $limit = -1,
    ...
    if ($groupmode) {
        ...
        // New! Modification: Select most recent user group and corresponding disucussion.
        $context = context_course::instance($cm->course);
        $usergroup = null;

        if (has_capability('moodle/site:accessallgroups', $context) or $groupmode == VISIBLEGROUPS) {
            ...
            if (!$usergroup) {
                ...
                print_error("The activity is in groupmode but the user is in no group.");
            } else {

                //$SESSION->activegroup[$cm->course][groups_get_activity_groupmode($cm)][$cm->groupingid] = $usergroup;
...
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost = false, $reply = false, $link = false,
    ...
    if (!empty($post->rating)) {
        // TODO: Check if user has already rated positively or negatively and set corresponding attribute such that either an upvote or downvote button can be displayed.
...
// $forum is an object
    ...
    if ($currentgroup === null) {
        ...
        $usergroup = null;
        if (has_capability('moodle/site:accessallgroups', $course_context) or $groupmode == VISIBLEGROUPS) {
...
// First check the group stuff
    if ($currentgroup == -1 or $groupmode == -1) {
        $groupmode = groups_get_activity_groupmode($cm, $course);
        //$currentgroup = groups_get_activity_group($cm);
        // Modification: Select most recent user group and corresponding disucussion.
        $context = context_course::instance($cm->course);
        $usergroup = null;
        if (has_capability('moodle/site:accessallgroups', $context) or $groupmode == VISIBLEGROUPS) {
