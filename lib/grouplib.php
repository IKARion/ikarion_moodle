...
/* New!
 *
 * This method is important for the modified Etherpad plugin and the current group block.
 * In Moodle one user can be member of different groups.
 * For some applications it is useful to get the most recent group of a user according to the time when
 * the group was created.
 */
function groups_get_most_recent_user_group($userid, $courseid) {

    global $DB;
    $params = array('userid1'=>$userid, 'courseid1'=>$courseid, 'userid2'=>$userid, 'courseid2'=>$courseid);

    $sql = "SELECT g.id FROM mdl_groups g, mdl_groups_members m
      where g.id =m.groupid and m.userid = :userid1 and courseid = :courseid1
      and timecreated = (SELECT max(g2.timecreated) FROM mdl_groups g2, mdl_groups_members m2 where g2.id = m2.groupid and m2.userid = :userid2 and courseid = :courseid2)";
