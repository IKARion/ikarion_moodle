...
$PAGE->set_url(new moodle_url('/blocks/analytics_recommendations/recommendations3.php', array('id' => $course->id, 'user' => $user->id)));
...
print_tabs($tabs, '', array('to_get_best_grade'), array('to_get_best_grade'));

// Page title
echo $OUTPUT->heading(get_string('best_grade','block_analytics_recommendations'));

// Information box
echo $OUTPUT->box(get_string('recommendations3_message','block_analytics_recommendations'),'generalbox','notice');
...
$table_data[2][0]=get_string('best_participation','block_analytics_recommendations');
$i=1;
foreach ($resumen_user_actual as $key=>$value) { 
    if (isset($mejores_normal[$key]))
        $participation=round($mejores_normal[$key]*100/$num_users,2);
    else $participation=0;
    if ($participation>100) $participation=100;
    $table_data[2][$i]=$participation.'%';
	$colours[2][$i]='#B0E0E6';
    $i++;            
}
...
$i=1;
foreach ($resumen_user_actual as $key=>$value) {
    if ($esfuerzo_mejor[$key]>100) $esfuerzo_mejor[$key]=100;
    $table_data[3][$i]=$esfuerzo_mejor[$key].'%';
	$colours[3][$i]='#FFA23C';
    $i++;                        
}
...
// Graphs
echo $gui->get_recommendations_graphs($course, $user, 4, 5);
